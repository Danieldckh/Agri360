...
(function () {
  ...
  var DEFAULT_AVATAR = 'data:image/svg+xml;base64,' + btoa('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="rgba(255,255,255,0.3)"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>');
  ...
  function renderConversationList(searchQuery) {
    ...
    // Apply custom folder filter
    if (activeFolderId && activeFolderChannelIds.length > 0) {
      allChannels = allChannels.filter(function (c) {
        return activeFolderChannelIds.indexOf(c.id) !== -1;
      });
    } else if (activeFolderId && activeFolderChannelIds.length === 0) {
      allChannels = [];
    }
  ...
  function loadCustomFolders() {
    apiGet('/custom-folders').then(function (data) {
      if (!Array.isArray(data)) {
        customFolders = [];
      } else {
        customFolders = data;
  ...
  function renderFoldersList() {
    var listEl = document.getElementById('msgFoldersList');
    if (!listEl) return;
    clearEl(listEl);

    if (customFolders.length === 0) {
      var empty = document.createElement('div');
      empty.className = 'msg-sidebar-folders-empty';
      empty.textContent = 'No folders yet';
      listEl.appendChild(empty);
      return;
    }

    customFolders.forEach(function (folder) {
      var row = document.createElement('div');
      row.className = 'msg-sidebar-folder-row';
      if (activeFolderId === folder.id) {
        row.classList.add('active');
      }

      var iconSpan = document.createElement('span');
      iconSpan.className = 'msg-sidebar-folder-icon';
      if (folder.emoji) {
        iconSpan.textContent = folder.emoji;
      } else if (folder.icon && window.EmojiIconPicker) {
        var svgEl = window.EmojiIconPicker.getIconSvg(folder.icon, 14);
        if (svgEl) {
      ...
      nameSpan.className = 'msg-sidebar-folder-name';
      nameSpan.textContent = folder.name;
      row.appendChild(nameSpan);

      if (folder.item_count > 0) {
        var countSpan = document.createElement('span');
        countSpan.className = 'msg-sidebar-folder-count';
        countSpan.textContent = String(folder.item_count);
  ...
  function selectFolder(folderId) {
    ...
    apiGet('/custom-folders/' + folderId + '/items').then(function (items) {
      if (!Array.isArray(items)) items = [];
      activeFolderChannelIds = items
        .filter(function (it) { return it.channel_id; })
        .map(function (it) { return it.channel_id; });
      renderFoldersList();
      renderConversationList('');
    }).catch(function () {
      activeFolderChannelIds = [];
  ...
  function showFolderContextMenu(e, folder) {
    var existing = document.querySelector('.msg-folder-ctx-menu');
    if (existing) existing.parentNode.removeChild(existing);

    var menu = document.createElement('div');
    menu.className = 'msg-folder-ctx-menu';
    ...
    renameBtn.className = 'msg-folder-ctx-item';
    ...
    deleteBtn.className = 'msg-folder-ctx-item msg-folder-ctx-item-danger';
    deleteBtn.textContent = 'Delete';
    deleteBtn.addEventListener('click', function () {
      document.body.removeChild(menu);
      apiDelete('/custom-folders/' + folder.id).then(function () {
        if (activeFolderId === folder.id) {
          activeFolderId = null;
          activeFolderChannelIds = [];
          renderConversationList('');
  ...
  function showRenameFolderModal(folder) {
    ...
    saveBtn.className = 'msg-modal-btn msg-modal-btn-create';
    ...
    saveBtn.addEventListener('click', function () {
      ...
      apiPatch('/custom-folders/' + folder.id, { name: newName }).then(function () {
        document.body.removeChild(overlay);
        loadCustomFolders();
      }).catch(function () {
        document.body.removeChild(overlay);
  ...
  function renderFolderCreateModal() {
    ...
    nameInput.type = 'text';
    nameInput.className = 'msg-modal-input';
    nameInput.placeholder = 'e.g. Important';
    nameRow.appendChild(nameInput);
    modal.appendChild(nameRow);
    ...
    createBtn.addEventListener('click', function () {
      ...
      apiPost('/custom-folders', body).then(function () {
        document.body.removeChild(overlay);
        loadCustomFolders();
      }).catch(function () {
        document.body.removeChild(overlay);
  ...
  function showAddToFolderMenu(e, channel) {
    var existing = document.querySelector('.msg-folder-ctx-menu');
    if (existing) existing.parentNode.removeChild(existing);
    ...
    var header = document.createElement('div');
    header.className = 'msg-folder-ctx-header';
    header.textContent = 'Add to folder';
    menu.appendChild(header);

    customFolders.forEach(function (folder) {
      var item = document.createElement('button');
      item.className = 'msg-folder-ctx-item';
      ...
      item.addEventListener('click', function () {
        if (menu.parentNode) document.body.removeChild(menu);
        apiPost('/custom-folders/' + folder.id + '/items', { channelId: channel.id }).then(function () {
          loadCustomFolders();
        }).catch(function () {});
      });
  ...
  function renderSidebarContent() {
    ...
    foldersSection.className = 'msg-sidebar-folders';

    var foldersHeader = document.createElement('div');
    foldersHeader.className = 'msg-sidebar-folders-header';

    var foldersTitle = document.createElement('span');
    foldersTitle.className = 'msg-sidebar-folders-title';
    foldersTitle.textContent = 'FOLDERS';
    foldersHeader.appendChild(foldersTitle);

    var addFolderBtn = createSvgButton('M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z', 'msg-sidebar-folder-add-btn', 14);
    addFolderBtn.title = 'Create folder';
    addFolderBtn.addEventListener('click', function () {
      renderFolderCreateModal();
    });
    foldersHeader.appendChild(addFolderBtn);

    foldersSection.appendChild(foldersHeader);

    var foldersList = document.createElement('div');
    foldersList.className = 'msg-sidebar-folders-list';
    foldersList.id = 'msgFoldersList';
    foldersSection.appendChild(foldersList);

    sidebarContent.appendChild(foldersSection);
    loadCustomFolders();
